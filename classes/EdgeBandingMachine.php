...
class EdgeBandingMachine
{
	...
	function returnNotCutOrders(){
		$orders = array();
		if($result = $this->dbo->query("SELECT `orders`.`id` as orderId, `orders`.`document_number`, DATE_FORMAT(`orders`.`order_completion_date`, '%d.%m') as completionDate, DATE_FORMAT(`orders`.`admission_date`, '%d.%m') as admissionDate, CONCAT_WS(' ', workers.`name`, workers.`surname`) as sellerName, `orders`.`customer_id`, `customers`.`name` as customerName, `customers`.`surname` as customerSurname, `customers`.`phone`, `orders`.`saw_number`, `customers_temp`.`name` as customerTempName, `customers_temp`.`phone` as customerTempPhone, `orders_comments`.`comments` as orderComment, GROUP_CONCAT(DISTINCT CONCAT(`boards_signs`.`sign`, (TRIM(`boards_thickness`.`thickness`)+0), '-', `boards_symbols`.`symbol`, COALESCE(`other_boards_symbols`.`symbol`, ''), ' (', (TRIM(`orders_boards`.`amount`)+0), ' szt.', ')') SEPARATOR '</br>') as boards FROM `orders` LEFT JOIN `customers_temp` ON `customers_temp`.`order_id`=`orders`.`id` LEFT JOIN `orders_comments` ON `orders_comments`.`order_id`=`orders`.`id`, `customers`, `workers`, `orders_boards` LEFT JOIN `other_boards_symbols` ON `other_boards_symbols`.`orders_boards_id`=`orders_boards`.`id`, `boards_signs`, `boards_thickness`, `boards_symbols` WHERE `orders`.`customer_id`=`customers`.`id` AND `orders`.`id` IN (SELECT `order_id` FROM `orders_boards` WHERE `cutting_completion_date` IS NULL) AND `orders`.`id` IN (SELECT `orders_boards`.`order_id` FROM `orders_boards` LEFT JOIN `edge_banding` ON `edge_banding`.`orders_boards_id`=`orders_boards`.`id` WHERE `edge_banding`.`edge_banding_metters_machine`=0) AND `workers`.`id`=`orders`.`worker_id` AND `orders_boards`.`order_id`=`orders`.`id` AND `orders_boards`.`board_sign_id`=`boards_signs`.`id` AND `orders_boards`.`board_thickness_id`=`boards_thickness`.`id` AND `orders_boards`.`board_symbol_id`=`boards_symbols`.`id` GROUP BY `orders`.`id` ORDER BY `order_completion_date` LIMIT 15")){
	...
	function returnOrderDetails($orderId){
		$boards = array();
		if($query = $this -> dbo -> prepare("SELECT ob.`id` as boardId, `boards_signs`.`sign` as boardSign, boardSymbols.`symbol` as boardSymbol, `other_boards_symbols`.`symbol` as otherSymbol, `boards_thickness`.`thickness`, ob.`amount`, ob.`cutting_metters`, DATE_FORMAT(ob.`cutting_completion_date`, '%d-%m-%Y') as cuttingDate, DATE_FORMAT(ob.`cutting_completion_date`, '%H:%i') as cuttingTime,`cutting_comments`.`comment` as cuttingComment, eb.`id` as edgeBandingId, `edge_band_sticker_symbols`.`symbol` as stickerSymbol, `edge_band_types`.`type` as edgeBandType, edgeBandSymbols.`symbol` as edgeBandSymbol, `eb`.`edge_banding_metters_wz` as wzMetters, `eb`.`edge_banding_metters_machine` as machineMetters, `eb`.`edge_banding_completion_date` as edgeBandingDate, `edge_band_comments`.`comments` as edgeBandComment, `edge_banding_comments`.`comments` as edgeBandingComment, GROUP_CONCAT(DISTINCT CONCAT_WS(' ', workers.`name`, workers.`surname`) SEPARATOR ',</br>')  as cuttingWorkers FROM `orders_boards` ob LEFT JOIN `other_boards_symbols` ON `other_boards_symbols`.`orders_boards_id`=ob.`id` LEFT JOIN `boards_symbols` as boardSymbols on boardSymbols.`id`=ob.`board_symbol_id` LEFT JOIN `cutting_comments` ON `cutting_comments`.`orders_boards_id`=ob.`id` LEFT JOIN `cutting_workers` ON `cutting_workers`.`orders_boards_id`=ob.`id` LEFT JOIN `workers` ON `cutting_workers`.`worker_id`=`workers`.`id`, `boards_signs`, `boards_thickness`, `edge_banding` eb LEFT JOIN `boards_symbols` as edgeBandSymbols on edgeBandSymbols.`id`=eb.`board_symbol_id` LEFT JOIN `edge_banding_comments` ON eb.`id`=`edge_banding_comments`.`edge_banding_id` LEFT JOIN `edge_band_comments` ON `edge_band_comments`.`edge_banding_id`=eb.`id`, `edge_band_sticker_symbols`, `edge_band_types` WHERE ob.`order_id`=:orderId AND ob.`board_sign_id`=`boards_signs`.`id` AND ob.`board_thickness_id`=`boards_thickness`.`id` AND `eb`.`orders_boards_id`=`ob`.`id` AND `eb`.`edge_band_sticker_symbol_id`=`edge_band_sticker_symbols`.`id` AND `eb`.`edge_band_type_id`=`edge_band_types`.`id` GROUP BY boardId, edgeBandingId")){
