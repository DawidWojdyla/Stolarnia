			...
			<tr class="board<?=$position -> boardId?>" id="<?=$position -> boardId?>">
				<td class="text-center" style="vertical-align: middle; border-top: 2px solid white;">
					<h3 class="noPadding noMargin" style="float: left; margin-top: 0px; margin-left: 10%;">
						...
						<span style="margin-left: 20px;" id="boardSign<?=$position -> boardId?>"><?=$position -> boardSign?></span><span id="boardThickness<?=$position -> boardId?>"><?=($position -> thickness)+0?></span><span id="hyphen<?=$position -> boardId?>"><?PHP if($position -> boardSymbolId != '1' || $position -> otherSymbol): ?> - <?PHP endif; ?></span><span id="boardSymbol<?=$position -> boardId?>"><?=$position -> boardSymbol?></span><span class="text-uppercase" id="otherBoardSymbol<?=$position -> boardId?>"><?=$position -> otherSymbol?></span>
					</h3>
