...
<?PHP if($smsContent): ?>smsContent = '<?=$smsContent?>';<?PHP endif; ?>
...
function showCuttingModal(boardId){
	if(document.getElementById('s'+boardId).innerHTML != 'pocięta'){ 
		...
		document.getElementById('cutting-modal-body').innerHTML = "<h4>"+boardName+"</h4><form id='boardCuttingForm' method='post'><input type='hidden' name='boardId' value="+boardId+"><table class='table table-condensed edgeBandingModalTable'><tr><td colspan='2'><label for='sawWorkers'>Wybierz pilarza:</label><select class='form-control' id='sawWorkers' name='sawWorkers[]' multiple required><?PHP foreach ($sawWorkers as $sawWorker): ?><option value='<?=$sawWorker->id?>'><?=$sawWorker->name?></option><?PHP endforeach; ?></select></td></tr><tr><td colspan='2'><label for='newC"+boardId+"'>Uwagi (opcjonalnie)</label><textarea class='form-control' id='newC"+boardId+"' name='boardCuttingComment' rows='3' cols='25' maxlength='250' form='boardCuttingForm'></textarea></td></tr><tr><td colspan='2'><div class='btn btn-default btn-block' onclick='sendBoardCuttingForm("+boardId+");'><span class=\"glyphicon glyphicon-floppy-disk\"></span> Zapisz</div><button id='sendingButton' type='submit' style='display:none;'></button><div class='btn btn-default btn-block' data-dismiss='modal' type='button'><span class='glyphicon glyphicon-remove'></span> Anuluj</div></td></tr></table></form>";
	}
	else{
