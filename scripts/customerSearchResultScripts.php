...
function showCustomerOptions(id){
	...
	var modalBody = "<div class='title1'><div>" + name + " " + surname + "</div><div class='title2'>tel. "+phone+"</div></div>";
	
	if(document.getElementById('isRemoved'+id).innerHTML == ""){
		...
		modalBody += "<form id='customerUpdatingForm' action='index.php?action=showCustomerUpdatingForm' method='post'>"+inputs+"<button type='submit' class='btn btn-default btn-block'><span class=\"glyphicon glyphicon-pencil\"></span> Edycja danych klienta</button></form><div class='btn btn-default btn-block' onclick='removeCustomer("+id+");'><span class=\"glyphicon glyphicon-trash\"></span> Usuń klienta</div>";
	}else{
		modalBody += "<div class='btn btn-default btn-block' onclick='restoreCustomer("+id+");'><span class=\"glyphicon glyphicon-refresh\"></span> Przywróć klienta</div>";
	}
...
function removeCustomerFromList(id){
	...
	ajaxRequest.done(function (response){
		switch(response){
			...
			document.getElementById('isRemoved' + id).innerHTML = "(usunięty)";
				break;
...
function removeCustomerFromRemovedCustomersList(id){
	...
	ajaxRequest.done(function (response){
		switch(response){
			...
			document.getElementById('isRemoved'+id).innerHTML = "";
				break;
			case 'FORM_DATA_MISSING': 
			case 'ACTION_FAILED': 
